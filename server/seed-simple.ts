  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      const packageType = await db.insert(schema.packageTypes).values({
        ...
        totalUses: randomElement([5, 10, 15]),
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const customerPackage = await db.insert(schema.customerPackages).values({
        ...
        validUntil: randomDate(new Date('2024-12-31'), new Date('2025-06-30')),
        status: 'active',
        purchasePrice: packageType.price,
      }).returning();
      
      customerPackages.push(customerPackage[0]);
    }
    ...
    for (let i = 0; i < 15; i++) {
      ...
      await db.insert(schema.appointments).values({
        ...
        petId: pet.id,
        serviceId: service.id,
        scheduledDate: randomDate(new Date('2024-11-15'), new Date('2024-12-31')),
        status: randomElement(['scheduled', 'confirmed', 'ready']),
    ...
    for (let i = 0; i < 10; i++) {
      ...
      await db.insert(schema.notifications).values({
        customerId: customer.id,
        type: randomElement(['confirmation', 'ready', 'reminder']),
    ...
    console.log(`   - 10 Package usages created`);
